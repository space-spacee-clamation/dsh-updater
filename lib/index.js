...
async function fetchExisting(url, branch, targetDir, force, run = defaultGitRunner) {
	...
	if (!force && await isDirty(targetDir, run)) throw new Error(`target checkout has uncommitted changes: ${targetDir}; commit/stash them or enable force`);
	await run([
		"fetch",
		"--quiet",
		"--no-tags",
		...
		return this.runExclusive(async () => {
			...
			try {
				...
				if (!existing) await cloneRemote(this.repoUrl, branch, targetDir, this.runGit);
					...
					if (!this.force && await isDirty(targetDir, this.runGit)) throw new Error(`target checkout has uncommitted changes: ${targetDir}; commit/stash them or enable force`);
